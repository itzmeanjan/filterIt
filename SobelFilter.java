                ...
                result.setRGB(j, i,
                        (new Color(
                                this.computeGradient(this.computePartial(
                                        this.applyMask(vMask, pxl.getNeighbouringPixelsFromImage(img, 'r', order))), 0),
                                this.computeGradient(this.computePartial(
                                        this.applyMask(vMask, pxl.getNeighbouringPixelsFromImage(img, 'g', order))), 0),
                                this.computeGradient(this.computePartial(
                                        this.applyMask(vMask, pxl.getNeighbouringPixelsFromImage(img, 'b', order))), 0))
                                                .getRGB()));
            }
        }
        return result;
        ...
        for (int i = 0; i < img.getHeight(); i++) {
            for (int j = 0; j < img.getWidth(); j++) {
                ...
                result.setRGB(j, i,
                        (new Color(
                                this.computeGradient(this.computePartial(
                                        this.applyMask(hMask, pxl.getNeighbouringPixelsFromImage(img, 'r', order))), 0),
                                this.computeGradient(this.computePartial(
                                        this.applyMask(hMask, pxl.getNeighbouringPixelsFromImage(img, 'g', order))), 0),
                                this.computeGradient(this.computePartial(
                                        this.applyMask(hMask, pxl.getNeighbouringPixelsFromImage(img, 'b', order))), 0))
                                                .getRGB()));
            }
        }
        return result;
