...
class SobelFilter implements Filter {
    ...
    private int computeGradient(int gx, int gy) {
        int grad = (int) Math.round(Math.sqrt(Math.pow(gx, 2) + Math.pow(gy, 2)));
        return grad % 256; // because images are 8bit RGB images, so we can't let intensity be > 255
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(sfFilter.filterAndSaveV("./sobelV.jpg", 1));
    }
}