...
/**
 * Actual worker class manages reversal of a column in a given image, gets
 * executed concurrently & as we're not updating same location of image from
 * multiple threads, there's no chance of getting inconsistent view of buffered
 * image
 */
class VerticalRotationWorker implements Runnable {
     ...
     * 
     * image[i][col] to be put into image[j][col]
     * 
     * &
     * 
     * image[j][col] to be put in place of image[i][col]
     * 
     * but point to be noted is that, we're not really touching source image, rather
     * modifications are getting performed in sink image
     */
