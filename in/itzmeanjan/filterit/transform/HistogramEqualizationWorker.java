 ...
 * & stores it into output image ( in supplied position )
 * 
 * Multiple pixels can be modified at same time, while each of them running in a
 * different thread of execution, concurrent
 */
class HistogramEqualizationWorker implements Runnable {
    ...
    HistogramEqualizationWorker(int i, int j, double minCDF, double[] cdfs, Color color, BufferedImage sink) {
        this.idxI = i;
        this.idxJ = j;
        this.minCDF = minCDF;
        this.cdfs = cdfs;
    ...
    /**
     * Pixel transformation using histogram equalization function, to be invoked
     * automatically by thread, as soon as task is submitted & worker thread is
     * available in thread pool
     */
    @Override
    public void run() {
        int transformedIntensity = this.transformPixel(this.minCDF, this.cdfs[this.color.getRed()], 255);
