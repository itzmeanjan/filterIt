...
/**
 * Worker, to be invoked for processing a pixel ( i.e. applying gamma correction
 * transformation function ), to be submitted to thread pool & when a thread is
 * available it'll get the job done.
 */
class GammaCorrectionWorker implements Runnable {
     ...
     * because pixel intensity can't be float
     */
    private int transformPixel(int intensity, int maxIntensity) {
        return (int) (255 * Math.pow(Math.E, this.gamma * Math.log(this.normalizeIntensity(intensity, maxIntensity))));
    }
