...
/**
 * Applies subtraction operation on two row of pixels, each from two different image of same
 * dimension; modified pixel pixel intensities are scaled into proper range & stored into sink
 * buffered image.
 *
 * <p>Each row processing done concurrently.
 */
class SubtractionWorker implements Runnable {
   ...
   * by subtracting ( not using absolute value, so no doubt clipping/ scaling is required ) them;
   * clips pixel values or scales them by using modulas operator, depending upon user supplied
   * argument
   */
  private Color subtract(Color a, Color b) {
    return this.clip
        ? new Color(
  ...
  public void run() {
    for (int i = 0; i < this.sink.getWidth(); i++)
      this.sink.setRGB(i, this.row, this.subtract(colorsOne[i], colorsTwo[i]).getRGB());
  }
}
